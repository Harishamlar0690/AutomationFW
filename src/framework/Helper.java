...
public class Helper{
	...
	File testDataFileLocation_Destination = new File(System.getProperty("user.dir")+"\\src\\utilfiles\\CapitalMarket.xlsx");
	...
	public void set_Data(LinkedHashMap<String, String> data)
	{
		allProcessData = data;
	}
	...
	public String data(String key) throws FrameworkException {		
		...
		if(Val==null)
		{
			report.log(LogStatus.FAIL,"Data issue- Column["+key+"] not found in the data sheet");
			throw new FrameworkException("Data issue- Column["+key+"] not found in the data sheet");
		}else {
			return Val;
		}
	}
	...
	public LinkedHashMap<String, String> configData(String configName) throws Exception{
		...
		recordset.next();
		for(String filed:recordset.getFieldNames()) {
			configData.put(filed, recordset.getField(filed));
		}
		recordset.close();		
		connection.close();
		return configData;
